...
*&---------------------------------------------------------------------*
*&       Class (Implementation)  mycheckbox
*&---------------------------------------------------------------------*
*        Text
*----------------------------------------------------------------------*
CLASS mycheckbox IMPLEMENTATION.
  METHOD on_click_checkbox.
    ...
    IF wa_mara-checkbox IS INITIAL.
      wa_mara-checkbox = 'X'.
    ELSE.
      CLEAR wa_mara-checkbox.
    ENDIF.
    MODIFY it_mara FROM wa_mara INDEX sy-tabix.
    myalv->refresh( ).
  ENDMETHOD.
ENDCLASS.               "mycheckbox
...
START-OF-SELECTION.
  ...
  TRY.
      ...
      mycolumn ?= mycolumns->get_column( 'CHECKBOX' ).
